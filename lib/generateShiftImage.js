...
// フォントファイルの絶対パスを構築
const fontPath = path.join(process.cwd(), "public", "fonts", "NotoSansCJKjp-Black.otf");

// フォントファイルが存在する場合のみ登録（本番対策）
if (fs.existsSync(fontPath)) {
    registerFont(fontPath, { family: "Noto Sans JP" });
} else {
    console.warn("⚠️ フォントファイルが見つかりませんでした:", fontPath);
}
